...
# 使用-f 输出更详细的格式化数据
ps -f 
# 列出所有进程 -e, 通常使用 ps -ef参数
ps -e
# 列出与当前terminal相关的所有进程 -a
ps -a
# 加参数-u USERNAME, 将显示该用户的进程
ps -u USERNAME
ps --sort -pcpu # -pcpu按cpu降序排列,+pcpu按升序排列
ps --sort -pmem # -pmem按mem降序排列,+pmem按升序排列
# 使用-C cmd过滤条件
ps -C COMMAND # 通过cmd字段检索进程(可能要完全吻合)
# 使用 -L PID 输出与该进程的子线程
ps -L PID
# 使用 -p 或 -q，显示pid列表范围内的进程
ps -p 31598 31589  或 ps -q 31589 31598


## 服务Service配置，现在逐步使用systemctl替代
chkconfig

## ulimit 查看系统的限制 
ulimit -n   # 查看文件描述符的限制数量
// 还有不少其它的设置
